...
fn text_alpha(pos: vec2f) -> f32 {
  var uv = pos / params.viewport.xy;
  uv = vec2f(uv.x, 1.0 - uv.y);
  let dimensions = vec2f(textureDimensions(text_texture));
  return textureLoad(text_texture, vec2i(uv * dimensions), 0).a;
}
...
fn simulate(@builtin(global_invocation_id) id: vec3u) {
  ...
  particles_out[index].pos_vel = vec4f(position, velocity);
  particles_out[index].life = vec4f(
    particle_time,
    particle_duration,
    particle_alpha,
    current.life.w
